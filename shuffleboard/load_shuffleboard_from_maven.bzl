def shuffleboard_version_conf_impl(repository_ctx):
    ...
    build_file = Label("@wpi_bazel_deps//shuffleboard/from_maven/maven_" + flattened_version + ":BUILD.bazel")
    repo_file = Label("@wpi_bazel_deps//shuffleboard/from_maven/maven_" + flattened_version + ":repo.bzl")
...
def load_shuffleboard_from_maven(version):
    shuffleboard_version_conf(name = "local_shuffleboard", version = version)
