"""
Loads the correct version of the SnobotSim Libraries
"""
...
def pathweaver_version_conf_impl(repository_ctx):
    """
    Rule for loading the specified version of the pathweaver libraries.

    Args:
        repository_ctx:
    """

    build_file = Label("@wpi_bazel_deps//pathweaver/from_source:BUILD.bazel")
    repo_file = Label("@wpi_bazel_deps//pathweaver/from_source:repo.bzl")
...
def load_pathweaver_from_source(local_repository_path = None, git_repository_args = None):
    ...
    pathweaver_version_conf(name = "local_pathweaver")
